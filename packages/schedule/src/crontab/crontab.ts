...
export class Crontab {
    ...
    static parse(expression: string, options?: TriggerOptions & InnerOptions) {
        ...
        if (Crontab.predefined[expression]) {
            expression = Crontab.predefined[expression]
        }
        ...
        const start = Crontab.fieldnames.length - raw_fields.length
        for (let i = 0, c = Crontab.fieldnames.length; i < c; i++) {
            const field = Crontab.fieldnames[i]
            const value = raw_fields[i - start] ?? Crontab.default_fields[i]
            fields.push(parse_field(value, field))
        }

        // Return schedule with parsed fields.
        return new Crontab(new ParsedFields(fields), options)
    }
