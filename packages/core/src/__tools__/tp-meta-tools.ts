...
export namespace MetaTools {
    ...
    export const ComponentMeta = TpMetaWrapper<TpComponentLike>(
        DI_TOKEN.component_meta, 'prototype_only')
    export const Instance = TpMetaWrapper<any>(
        DI_TOKEN.instance, 'prototype_only')
    export const UnitRecord = TpMetaWrapper<TpUnitRecord>(
        DI_TOKEN.unit_record, 'prototype_only', () => new Map())
    export const Dependencies = TpMetaWrapper<Map<string | symbol, any[]>>(
        DI_TOKEN.dependencies, 'both', () => new Map())
    export const ClassMeta = TpMetaWrapper<ClassMeta>(
        DI_TOKEN.class_meta, 'prototype_only', () => ({ parameter_injection: [], loader: '', type: '' }))
    export const PropertyMeta = TpMetaWrapper<PropertyMeta>(
        DI_TOKEN.property_meta, 'property_only', () => ({ parameter_injection: [] }))
    export const CustomData = TpMetaWrapper<Map<string | symbol, any>>(
        DI_TOKEN.custom_data, 'prototype_only', () => new Map())
    export const PluginMeta = TpMetaWrapper<PluginMeta>(
        DI_TOKEN.plugin_meta, 'prototype_only', () => ({ loader_list: [], type: '', option_key: '' }))
