...
describe('static case', function() {
    ...
    beforeAll(async function() {
        ...
        http_static = platform.expose(HttpStatic)!
    })
    ...
    describe('serve static', function() {
        ...
        it('should serve static file', async function() {
            await r.get('/assets/some.txt').then(res => {
                ...
                expect(res.headers['content-length']).toEqual('8')
                expect(res.data.trim()).toEqual('abcdefg')
            })
        })
        ...
        it('should response with status 404 if specified file not exists', async function() {
            await r.get('/assets/not-exist.txt')
                ...
                .then(res => {
                    expect(res.status).toEqual(404)
                    expect(res.data).toEqual({ error: { status: 404, code: '404', msg: 'Not Found', headers: {}, stack: '' } })
                })
        })
        ...
        it('should response with status 403 if configure as deny dotfile', async function() {
            await r.get('/assets/.dotfile')
                ...
                .then(res => {
                    ...
                    expect(res.data).toEqual({ error: { status: 403, code: '403', msg: 'Forbidden', headers: {}, stack: '' } })
                })
        })
        ...
        it('should response as normal if configure as allow dotfile', async function() {
            ...
            await r.get('/assets/.dotfile')
                ...
                .then(res => {
                    expect(res.status).toEqual(200)
                    expect(res.data.trim()).toEqual('dotfile content')
                })
        })
    })
    ...
    describe('#create_stream()', function() {
        ...
        it('should read exist file as ReadStream', async function() {
            const stream = await create_stream('./tests/assets/some.txt')
            expect(stream).toBeInstanceOf(ReadStream)
        })

        it('should read non-exist file with a TpHttpError', async function() {
            await expect(create_stream('./tests/assets/non-exists.txt')).rejects.toThrow(TpHttpFinish)
        })
    })
    ...
    describe('#constructor()', function() {
        ...
        it('should startup with default config', function() {
            ...
            expect(http_static).toHaveProperty('static_configs')
            expect(http_static.static_configs).toHaveProperty('')
            expect(http_static.static_configs['']).toHaveProperty('scope')
            expect(http_static.static_configs[''].scope).toEqual('')
            expect(http_static.static_configs['']).toHaveProperty('root')
            expect(http_static.static_configs[''].root).toEqual(process.cwd())
            expect(http_static.static_configs['']).toHaveProperty('cache_size')
            expect(http_static.static_configs[''].cache_size).toEqual(100)
        })

        it('should throw error if specified root path is not a directory', function() {
            expect(() => new HttpStatic(new TpConfigData({ http: { port: 3939, static: { root: './tests/assets/some.txt' } } }))).toThrow()
        })
    })
    ...
    describe('#is_fresh()', function() {
        ...
        it('should return false if Cache-Control contains no-cache', function() {
            ...
            expect(is_fresh(request, response)).toBe(false)
        })
        ...
        it('should return true if If-None-Match is *', function() {
            ...
            expect(is_fresh(request, response)).toBe(true)
        })
    ...
    describe('#is_precondition_failure()', function() {
        ...
        it('should return true if If-Match is *', function() {
            ...
            expect(is_precondition_failure(request, response)).toBe(false)
        })

        it('should return true if Etag not exists', function() {
            const { request, response } = mock_request_and_response({ if_match: '*' })
            expect(is_precondition_failure(request, response)).toBe(true)
        })
