...
describe('http-server.ts', function() {
    ...
    describe('HttpServer', function() {
        ...
        it('should start and terminate', async function() {
            const platform = new Platform(load_config({ http: { port: 0 } }))
                .import(HttpServerModule)
            const http_server = platform.expose(HttpServer)!
            expect(http_server.starting).toBeUndefined()
            expect(http_server.terminating).toBeUndefined()
            const starting = http_server.start(
                async (_req, _res) => undefined,
                async (_req, _socket, _head) => undefined)
            await starting
            expect(http_server.starting).toEqual(starting)
            expect(http_server.terminating).toBeUndefined()
            expect(http_server.server).toBeInstanceOf(Server)
            expect(http_server.websocket_server).toBeInstanceOf(WebSocketServer)
            const terminating = http_server.terminate()
            expect(http_server.terminating).toEqual(terminating)
            await terminating
        })
